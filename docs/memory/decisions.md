...
| DEC-GOV-007 | 2026-07-22 | Accepted | Adopt the hybrid agent organization in `docs/engineering/agent-organization.md`: role-based governance + code-ownership agent boundaries; product-acceptance QA split from reliability QA; agents instantiated lazily per active milestone. | Governance and decision-rights value of a full org chart without the cost/drift of instantiating agents for far-off milestones, and without duplicating the authoritative `docs/`. Code-ownership boundaries keep parallel agents collision-free and honour DEC-ARCH-010 (file I/O stays in Rust, not with the timeline agent). |
